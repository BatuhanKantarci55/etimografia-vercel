...
export interface UserProfileTabsRef {
  scrollToIndex: (index: number) => void;
}

const tabs = [
  { key: "gönderi", icon: "newspaper-outline" as const },
  { key: "takipçi", icon: "people-outline" as const },
  { key: "rozet", icon: "ribbon-outline" as const },
  { key: "sıralama", icon: "trophy-outline" as const },
];

const UserProfileTabs = forwardRef<UserProfileTabsRef, UserProfileTabsProps>(
  ({ userId, activeTab, onTabPress }, ref) => {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={{ width: CONTENT_WIDTH }}>
              <UserProfileContent tabKey={item.key} userId={userId} />
            </View>
          )}
        />
      </View>
